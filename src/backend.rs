...
pub fn format_monitorrule(out: &Output) -> Option<String> {
    if !out.enabled {
        return None;
    }
    ...
    Some(format!(
        "monitorrule=name:{},width:{},height:{},refresh:{:.6},x:{},y:{},scale:{:.6},rr:{},vrr:{}",
        out.name,
        w,
        h,
        r,
...
pub fn save_config(
    ...
    for out in outputs {
        if let Some(rule) = format_monitorrule(out) {
...
mod tests {
    ...
    fn out(vrr: bool, enabled: bool) -> Output {
        Output {
            name: "DP-1".into(),
    ...
    #[test]
    fn monitorrule_includes_vrr_flag() {
        assert_eq!(
            format_monitorrule(&out(true, true)).unwrap(),
            "monitorrule=name:DP-1,width:2560,height:1440,refresh:144.000000,x:10,y:20,scale:1.500000,rr:1,vrr:1"
        );
        assert!(
            format_monitorrule(&out(false, true))
                .unwrap()
                .ends_with(",vrr:0")
        );
    }
    ...
    #[test]
    fn disabled_output_has_no_rule() {
        assert!(format_monitorrule(&out(true, false)).is_none());
    }
}
